-- ============================================================
-- EverDream — Database Setup Script (Idempotent)
...
CREATE POLICY "Users can update own profile" ON public.profiles FOR UPDATE USING (auth.uid() = auth_user_id) WITH CHECK (auth.uid() = auth_user_id);
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'dreams') THEN
    ALTER TABLE public.dreams ENABLE ROW LEVEL SECURITY;
  END IF;
END $$;

DROP POLICY IF EXISTS "Users can view own dreams" ON public.dreams;
CREATE POLICY "Users can view own dreams" ON public.dreams FOR SELECT
...
CREATE POLICY "Users can insert own dreams" ON public.dreams FOR INSERT
...
CREATE POLICY "Users can update own dreams" ON public.dreams FOR UPDATE
  USING (user_id IN (SELECT id FROM public.profiles WHERE auth_user_id = auth.uid()))
  WITH CHECK (user_id IN (SELECT id FROM public.profiles WHERE auth_user_id = auth.uid()));

DROP POLICY IF EXISTS "Users can delete own dreams" ON public.dreams;
CREATE POLICY "Users can delete own dreams" ON public.dreams FOR DELETE
  USING (user_id IN (SELECT id FROM public.profiles WHERE auth_user_id = auth.uid()));

DROP POLICY IF EXISTS "Public dreams are viewable" ON public.dreams;
CREATE POLICY "Public dreams are viewable" ON public.dreams FOR SELECT
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'sleep_sessions') THEN
    ALTER TABLE public.sleep_sessions ENABLE ROW LEVEL SECURITY;
  END IF;
END $$;

DROP POLICY IF EXISTS "Users can view own sleep sessions" ON public.sleep_sessions;
CREATE POLICY "Users can view own sleep sessions" ON public.sleep_sessions FOR SELECT
  USING (user_id IN (SELECT id FROM public.profiles WHERE auth_user_id = auth.uid()));

DROP POLICY IF EXISTS "Users can insert own sleep sessions" ON public.sleep_sessions;
CREATE POLICY "Users can insert own sleep sessions" ON public.sleep_sessions FOR INSERT
  WITH CHECK (user_id IN (SELECT id FROM public.profiles WHERE auth_user_id = auth.uid()));

DROP POLICY IF EXISTS "Users can update own sleep sessions" ON public.sleep_sessions;
CREATE POLICY "Users can update own sleep sessions" ON public.sleep_sessions FOR UPDATE
  USING (user_id IN (SELECT id FROM public.profiles WHERE auth_user_id = auth.uid()))
  WITH CHECK (user_id IN (SELECT id FROM public.profiles WHERE auth_user_id = auth.uid()));

DROP POLICY IF EXISTS "Users can delete own sleep sessions" ON public.sleep_sessions;
CREATE POLICY "Users can delete own sleep sessions" ON public.sleep_sessions FOR DELETE
  USING (user_id IN (SELECT id FROM public.profiles WHERE auth_user_id = auth.uid()));
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'nfts') THEN
    ALTER TABLE public.nfts ENABLE ROW LEVEL SECURITY;
  END IF;
END $$;

DROP POLICY IF EXISTS "Users can view own nfts" ON public.nfts;
CREATE POLICY "Users can view own nfts" ON public.nfts FOR SELECT
  USING (user_id IN (SELECT id FROM public.profiles WHERE auth_user_id = auth.uid()));

DROP POLICY IF EXISTS "Users can insert own nfts" ON public.nfts;
CREATE POLICY "Users can insert own nfts" ON public.nfts FOR INSERT
  WITH CHECK (user_id IN (SELECT id FROM public.profiles WHERE auth_user_id = auth.uid()));

DROP POLICY IF EXISTS "Users can update own nfts" ON public.nfts;
CREATE POLICY "Users can update own nfts" ON public.nfts FOR UPDATE
  USING (user_id IN (SELECT id FROM public.profiles WHERE auth_user_id = auth.uid()));
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'dream_assets') THEN
...
CREATE POLICY "Users can update own dream assets" ON public.dream_assets FOR UPDATE
  USING (user_id IN (SELECT id FROM public.profiles WHERE auth_user_id = auth.uid()));
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'sync_log') THEN
    ALTER TABLE public.sync_log ENABLE ROW LEVEL SECURITY;
  END IF;
END $$;

DROP POLICY IF EXISTS "Users can view own sync log" ON public.sync_log;
CREATE POLICY "Users can view own sync log" ON public.sync_log FOR SELECT
  USING (user_id IN (SELECT id FROM public.profiles WHERE auth_user_id = auth.uid()));

DROP POLICY IF EXISTS "Users can insert sync log" ON public.sync_log;
CREATE POLICY "Users can insert sync log" ON public.sync_log FOR INSERT
  WITH CHECK (user_id IN (SELECT id FROM public.profiles WHERE auth_user_id = auth.uid()));
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'webhook_events') THEN
    ALTER TABLE public.webhook_events ENABLE ROW LEVEL SECURITY;
  END IF;
END $$;

DROP POLICY IF EXISTS "Users can view own webhooks" ON public.webhook_events;
CREATE POLICY "Users can view own webhooks" ON public.webhook_events FOR SELECT
  USING (user_id IN (SELECT id FROM public.profiles WHERE auth_user_id = auth.uid()));

DROP POLICY IF EXISTS "Users can insert webhooks" ON public.webhook_events;
CREATE POLICY "Users can insert webhooks" ON public.webhook_events FOR INSERT
...
CREATE TRIGGER set_updated_at BEFORE UPDATE ON public.dreams FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();

DROP TRIGGER IF EXISTS set_updated_at ON public.sleep_sessions;
CREATE TRIGGER set_updated_at BEFORE UPDATE ON public.sleep_sessions FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();
...
CREATE TRIGGER set_dreams_expires_at BEFORE INSERT ON public.dreams FOR EACH ROW EXECUTE FUNCTION public.set_expires_at();

DROP TRIGGER IF EXISTS set_sleep_sessions_expires_at ON public.sleep_sessions;
CREATE TRIGGER set_sleep_sessions_expires_at BEFORE INSERT ON public.sleep_sessions FOR EACH ROW EXECUTE FUNCTION public.set_expires_at();
...
RETURNS void AS $$
BEGIN
  DELETE FROM public.dream_assets WHERE dream_id IN (
    SELECT id FROM public.dreams WHERE is_deleted = true AND updated_at < NOW() - INTERVAL '7 days'
  );
...
GRANT SELECT ON public.dreams TO anon;  -- For public dreams
...
BEGIN
  ...
  WHERE table_schema = 'public'
    AND table_name IN ('profiles', 'dreams', 'sleep_sessions', 'user_settings', 'nfts', 'dream_assets', 'sync_log', 'webhook_events');

  RAISE NOTICE 'EverDream schema: % / 8 tables created', table_count;
END $$;
